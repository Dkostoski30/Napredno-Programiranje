...
class LogSystem<T extends ILog> implements LogProcessor<T>{
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = (logs) -> processLogs(logs.stream().
                filter(log -> log.getType()
                        .equals("INFO"))
                .collect(Collectors.toCollection(ArrayList<T>::new)));

        LogProcessor<T> secondLogProcessor = (logs) -> processLogs(logs.stream().
                filter(log -> log.getMessage().length()<100)
                .collect(Collectors.toCollection(ArrayList<T>::new)));

        LogProcessor<T> thirdLogProcessor = (logs) -> processLogs(logs.stream().sorted().collect(Collectors.toCollection(ArrayList<T>::new)));

...
class GenericCollection<T extends IHasTimestamp & Comparable<T>>{
    Map<String, List<T>> elementsMAP;

    public GenericCollection() {
        this.elementsMAP = new HashMap<>();
    }
    public void addGenericItem(String Category, T element){
        this.elementsMAP.putIfAbsent(Category, new ArrayList<>());
        this.elementsMAP.get(Category).add(element);
    }
    ...
    public Map<String, Set<T>> byMonthAndDay(){
        Map<String, Set<T>> resultMap = new TreeMap<>();
        elementsMAP.values().stream()
                .flatMap(List::stream)
                .map(element -> String.format("%02d-%02d", element.getTimestamp().getMonth().getValue(), element.getTimestamp().getDayOfMonth()))
                .distinct()
                .forEach(date -> {
                    resultMap.put(date, new TreeSet<>());
                });
        resultMap.keySet().forEach(key -> {
            elementsMAP.values()
                    .stream()
                    .flatMap(List::stream)
                    .filter(element -> String.format("%02d-%02d", element.getTimestamp().getMonth().getValue(), element.getTimestamp().getDayOfMonth()).equals(key))
                    .forEach(element -> resultMap.get(key).add(element));
        });
        return resultMap;
    }
    public Map<Integer, Long> countByYear(){
        ...
        elementsMAP.values()
                .stream()
                .flatMap(List::stream)
                .map(element -> element.getTimestamp().getYear())
                .distinct()
                .forEach(year -> {
                    resultMap.put(year, 0L);
                });
        resultMap.keySet().forEach(key -> {
            resultMap.put(key, elementsMAP.values()
                    .stream()
                    .flatMap(List::stream)
                    .filter(element -> key.equals(element.getTimestamp().getYear()))
                    .count());
        });
        return resultMap;
    }
    public Collection<T> itemsFromCategories (List<String> categories){
        return elementsMAP.entrySet()
                .stream().filter(entry -> categories.contains(entry.getKey()))
...
public class GenericCollectionTest {
    ...
    private static void printResultsFromItemsFromCategories(
            GenericCollection<? extends IHasTimestamp> collection, List<String> categories) {
